...
/*  61 */       superClass = theClass.getSuperclass();
/*  62 */       if (!superClass.getKind().equals(TypeKind.NONE))
/*  63 */         retClass = getDeclaringClassMethod(superClass, methodName, args);
/*     */     }
/*  65 */     if (retClass == null)
/*  66 */       for (superClass = theClass.getInterfaces().iterator(); superClass.hasNext(); ) { interfaceType = (TypeMirror)superClass.next();
/*  67 */         retClass = getDeclaringClassMethod(interfaceType, methodName, args);
/*     */       }
/*     */     TypeMirror interfaceType;
/*  70 */     if (retClass == null) {
/*  71 */       Collection methods = ElementFilter.methodsIn(theClass.getEnclosedElements());
...
/*     */   {
/*  84 */     Collection interfaces = type.getInterfaces();
...
/* 106 */       Collection argTypes = ((DeclaredType)type).getTypeArguments();
/* 107 */       if (argTypes.size() == 1)
/* 108 */         return (TypeMirror)argTypes.iterator().next();
...
/* 142 */       superClass = d1.getSuperclass();
 ...
 * Qualified Name:     com.sun.tools.internal.ws.processor.modeler.annotation.TypeModeler
