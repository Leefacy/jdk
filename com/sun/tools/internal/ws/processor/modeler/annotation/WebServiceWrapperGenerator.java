...
/* 113 */   private static final FieldFactory FIELD_FACTORY = new FieldFactory(null);
/*     */ 
/* 115 */   private final AbstractWrapperBeanGenerator ap_generator = new ApWrapperBeanGenerator(InlineAnnotationReaderImpl.theInstance, new ApNavigator(this.builder
...
/*     */ 
/* 233 */     operationName = (webMethod != null) && (webMethod.operationName().length() > 0) ? webMethod
/* 233 */       .operationName() : operationName;
...
/* 240 */     RequestWrapper reqWrapper = (RequestWrapper)method.getAnnotation(RequestWrapper.class);
...
/*     */ 
/* 252 */     File file = new File(DirectoryUtil.getOutputDirectoryFor(requestClassName, this.builder.getSourceDir()), 
/* 252 */       Names.stripQualifier(requestClassName) + 
/* 252 */       GeneratorConstants.JAVA_SRC_SUFFIX.getValue());
...
/* 268 */       ResponseWrapper resWrapper = (ResponseWrapper)method.getAnnotation(ResponseWrapper.class);
...
/*     */       }
/*     */ 
/* 286 */       file = new File(DirectoryUtil.getOutputDirectoryFor(responseClassName, this.builder.getSourceDir()), 
/* 286 */         Names.stripQualifier(responseClassName) + 
/* 286 */         GeneratorConstants.JAVA_SRC_SUFFIX.getValue());
...
/*     */ 
/* 320 */       List reqMembers = this.ap_generator.collectRequestBeanMembers(method);
/* 321 */       List resMembers = this.ap_generator.collectResponseBeanMembers(method);
/*     */ 
...
/*     */   {
/* 349 */     return (TypeMirror)this.makeSafeVisitor.visit(type, this.builder.getProcessingEnvironment().getTypeUtils());
/*     */   }
/*     */ 
...
/*     */   {
/* 378 */     List jaxbAnnotations = memInfo.getJaxbAnnotations();
...
/* 432 */     WebFault webFault = (WebFault)thrownDecl.getAnnotation(WebFault.class);
/* 433 */     String className = beanPackage + exceptionName + WebServiceConstants.BEAN.getValue();
/*     */ 
/* 435 */     Collection members = this.ap_generator.collectExceptionBeanMembers(thrownDecl);
...
/* 442 */       FaultInfo faultInfo = new FaultInfo(TypeMonikerFactory.getTypeMoniker(beanType), true);
/*     */ 
/* 444 */       namespace = webFault.targetNamespace().length() > 0 ? webFault
/* 444 */         .targetNamespace() : namespace;
/*     */ 
/* 446 */       name = webFault.name().length() > 0 ? webFault
/* 446 */         .name() : name;
/* 447 */       faultInfo.setElementName(new QName(namespace, name));
/* 448 */       this.seiContext.addExceptionBeanEntry(thrownDecl.getQualifiedName(), faultInfo, this.builder);
/* 449 */       return false;
/*     */     }
/* 451 */     if (webFault != null)
/*     */     {
/* 453 */       namespace = webFault.targetNamespace().length() > 0 ? webFault
/* 453 */         .targetNamespace() : namespace;
/*     */ 
/* 455 */       name = webFault.name().length() > 0 ? webFault
/* 455 */         .name() : name;
/*     */ 
/* 457 */       className = webFault.faultBean().length() > 0 ? webFault
/* 457 */         .faultBean() : className;
/*     */     }
/*     */ 
...
/*     */ 
/* 488 */     XmlType xmlType = (XmlType)thrownDecl.getAnnotation(XmlType.class);
...
/* 500 */     WebFault webFault = (WebFault)thrownDecl.getAnnotation(WebFault.class);
...
/* 538 */       paramArray = xmlTypeAnn.paramArray("propOrder");
...
/*     */ 
/* 561 */     m = cls.method(1, this.cm.VOID, "set" + accessorName);
...
/*     */   {
/*     */     protected ApWrapperBeanGenerator(Navigator<TypeMirror, TypeElement, ?, ExecutableElement> annReader, AbstractWrapperBeanGenerator.BeanMemberFactory<TypeMirror, MemberInfo> nav)
/*     */     {
/* 124 */       super(nav, beanMemberFactory);
/*     */     }
/*     */ 
 ...
 * Qualified Name:     com.sun.tools.internal.ws.processor.modeler.annotation.WebServiceWrapperGenerator
